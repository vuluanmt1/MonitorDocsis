...
public class AdapterTicketWarring extends RecyclerView.Adapter<AdapterTicketWarring.ViewHolder>implements Filterable {
    private Context mContext;
    private List<ModelTicketWarring>mList;
    private List<ModelTicketWarring>mNewList;

    public AdapterTicketWarring(Context mContext, List<ModelTicketWarring> mList) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(mContext).inflate(R.layout.ticket_warring_table,parent,false);
            ViewHolder holder = new ViewHolder(view);
            return holder;
    }
    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ModelTicketWarring item = mList.get(position);
    ...
    private Filter exFilter  = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            List<ModelTicketWarring> filteredList = new ArrayList<>();
            if(constraint==null ||constraint.length()==0){
                filteredList.addAll(mNewList);
            }else{
                String filterPattern = constraint.toString().toLowerCase().trim();
                for(ModelTicketWarring item :mNewList){
